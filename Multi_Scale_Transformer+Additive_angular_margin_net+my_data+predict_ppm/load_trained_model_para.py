...
import pandas as pd
from  handle_predicted_results import handle_results
...
def predict(model, device, test_loader, memory=None):
    ...
    for batch_size, (test_x, test_y) in enumerate(test_loader):
        ...
        test_x = test_x.view(test_x.shape[0],  test_x.shape[1])
...
def load_trained_model_para():
    ...
    if op.exists(des1) and op.exists(des2) and op.exists(des3):
        # 如果文件已经存在,先删除再保存
        os.remove(des1)
        os.remove(des2)
        os.remove(des3)
        print('已经删除文件,正在保存新文件')
