...
from Func import get_trainData, get_testData, get_updateData,draw, get_data
from adaptive_weight import *
import time
import torch

final_train_acr = 0.96
samples_num = 160
...
def check(X,y,device,model):
    X= X.view(X.shape[0],X.shape[2],X.shape[1])
    X= X.to(device)
    y= y.to(device)
    output = model.forward(X)
    predicted_y = torch.max(output,1)[1]
    # print(predicted_y)
    # print(y)
    i=0
    if predicted_y == y:
...
def getUpdatedIndexs(data_loader,device,model,indexs):
    i = 0
    # batch_size=1,shuffle=False
    for batch_index, (X, y) in enumerate(data_loader):
        if check(X,y,device,model) is True:
            # 置为-1代表, 这些样本的权重不需要更新,就保持1,不需要+=beta
            indexs[i] = -1
            i += 1
    return indexs


# getUpdatedIndexs(X_train+X_test,y_train+y_test)
def train(model=None, device=None, data=None, optimizer=None, loss_func=None, epochs=None,
          print_batch=None, scheduler=None,batch_size=None):
    loss_list_epoch = {'index': [], 'loss': [], 'test acr': [], 'train acr': []}
    lr_list = {'learning_rate': []}
    model = model.to(device)
    weights = [ 1 for i in range(0,samples_num)]
    f_t_a = 0  # 记录达到的最高训练准确率
    for epoch in range(epochs):
        # 每个epoch都需要重新抽取样本数据,保证模型优先训练漂移程度较大的样本(即用未漂移前模型预测效果出现误差的那些样本)
        X_train, X_test, y_train, y_test,indexs = get_data(weights)
        ...
        with tqdm(train_loader, total=len(train_loader)) as t:  # 🌟 1. 定义进度条
            for batch_index, (train_x, train_y) in enumerate(t):
                ...
                if f_t_a > final_train_acr:
                    break
        # 根据预测是否准确,调整样本集权重weights
        # 不需要设置batch_size,就默认为1
        update_loader = get_updateData(data_x=X_train+X_test,data_y=y_train+y_test)
        indexs = getUpdatedIndexs(update_loader,device,model,indexs)
        weights = updateWeights(weights,indexs)
