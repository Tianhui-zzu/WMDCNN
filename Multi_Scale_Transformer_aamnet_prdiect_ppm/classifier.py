...
class PartialFC(torch.nn.Module):
    def __init__(self, feature_size=112 * 8, vector_dim=112, class_num=4, ppm_num=10, m_f=None, p_r=None):
        ...
        self.memory = m_f  # 存储全连接层过后的特征向量
        # 8*1*14,回归任务，得出一个值代表浓度值
        self.down = nn.Linear(feature_size // 8, num_x)
        self.out = nn.Linear(num_x, 1)
        self.predict_results = p_r
    ...
    def forward(self, feature_vector):
        ...
        feature_vector = self.down(feature_vector)
        if self.memory != None:
            temp = feature_vector.view(feature_vector.size(0), -1).detach().cpu().numpy()
            # print('x.type',type(x))
            # print('temp-------',temp)
        ...
        # out =  torch.round(self.out(norm_feature),decimals=1)
        out = self.out(norm_feature)
        print('--------------out------', out)
        if self.predict_results != None:
            temp = out.view(out.size(0), -1).detach().cpu().numpy()
            # print('x.type',type(x))
            # print('temp-------',temp)
            self.predict_results.append(temp)
        return out
