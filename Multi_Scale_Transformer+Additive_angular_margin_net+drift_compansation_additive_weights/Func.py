...
from  adaptive_weight import getData_byWeights
from sklearn.model_selection import train_test_split

print_folder_path01 = '../handled_data1_1hz_expand_t1+t2_x-min'


def get_data(weights=None):
    ls_data = []
    ls_label = []
    ...
    if weights is not None:
        ls_data,ls_label,indexs =getData_byWeights(ls_data,ls_label,weights,num_samples=80)

    X_train, X_test, y_train, y_test = train_test_split(ls_data, ls_label, test_size=0.1, random_state=42)

    return X_train, X_test, y_train, y_test,indexs
...
def get_updateData(batch_size =1,data_x=None, data_y=None):
    update_x = torch.as_tensor(data_x, dtype=torch.float32)
    update_y = torch.as_tensor(data_y, dtype=int)
    update_data = Data.TensorDataset(update_x, update_y)
    # shuffle为False是为了保持dataloader里的数据和index下标对应
    update_dataloader = Data.DataLoader(dataset=update_data, batch_size=batch_size, shuffle=False)
    return update_dataloader
