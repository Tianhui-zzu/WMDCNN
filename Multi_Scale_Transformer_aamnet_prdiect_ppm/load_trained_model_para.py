...
if __name__ == '__main__':
    ...
    predict_results = []
    y_memory = []
    model = MST_AAMN(memory_t=x_memory_t, memory_f=x_memory_f, p_r=predict_results)
    model.to(device)
    # 初始化为已经训练完成的模型参数
    load_par(model)
    ...
    model.eval()
    final_mse = predict(model, device, testData, y_memory)
    print(final_mse)
    ...
    # print(y_memory.head())
    des1 = 'extract_x.csv'
    des2 = 'extract_x_f-b1.csv'
    des3 = 'extract_predicted_results.csv'
    des4 = 'extract_y.csv'
    if op.exists(des1):
        print('当前文件已存在')
    else:
        x_memory_t.to_csv(des1, index=False, header=None)
        x_memory_f.to_csv(des2, index=False, header=None)
        predict_results.to_csv(des3, index=False, header=None)
        y_memory.to_csv(des4, index=False, header=None)
        print('输出文件成功')
