...
def train(model, device, train_loader, test_loader, optimizer, loss_func, epochs, print_batch):
    ...
    for epoch in range(epochs):
        with tqdm(train_loader, total=len(train_loader)) as t:  # 🌟 1. 定义进度条
            for batch_index, (train_x, train_y) in enumerate(t):
                ...
                train_x = train_x.reshape(train_x.shape[0], 1, 128)
                ...
                time.sleep(0.00001)
    return loss_list_epoch  # 返回一个列表，记录训练过程中的所有的数据
