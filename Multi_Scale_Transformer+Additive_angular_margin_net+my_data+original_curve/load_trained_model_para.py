...
def draw_confusion_matrix(confusion_matrix):
    ...
    plt.xticks(range(len(confusion_matrix_normalization)), ['NH3', 'H2S', 'CS2', 'CS2_NH3','H2S_NH3','CS2_H2S'],rotation=30)
    plt.yticks(range(len(confusion_matrix_normalization)), ['NH3', 'H2S', 'CS2', 'CS2_NH3','H2S_NH3','CS2_H2S'],rotation=30)
    plt.xlabel('Predicted label')
    plt.ylabel('True label')
    # 通过调用 colorbar 函数来加上数值和颜色的对应规则
    plt.colorbar()
    plt.title('the confusion matrix of MST_AAM')
    plt.show()
...
if __name__ == '__main__':
    ...
    testData = get_testData(batch_size=10, data_x=X_test + X_train, data_y=y_test + y_train)
    model.eval()
    ...
    print('x_memory_t.shape',x_memory_t.shape)
    print('x_memory_f.shape',x_memory_f.shape)
    print('y_memory.shape',y_memory.shape)
    # 计算混淆矩阵
