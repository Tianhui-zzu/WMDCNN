...
def load_par(model):
    model.load_state_dict(torch.load('MST_AAMN_0.95.pth'))
...
def draw_confusion_matrix(confusion_matrix):
    ...
    plt.xticks(range(len(confusion_matrix_normalization)), ['Me', 'CO', 'Et', 'Et_Me', 'Et_CO'])
    plt.yticks(range(len(confusion_matrix_normalization)), ['Me', 'CO', 'Et', 'Et_Me', 'Et_CO'])
